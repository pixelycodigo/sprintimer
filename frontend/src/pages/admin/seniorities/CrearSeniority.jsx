...
export default function CrearSeniority() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await senioritiesService.crear({
        ...formData,
        orden: parseInt(formData.orden),
      });
      setSuccess('Seniority creado exitosamente');
      setTimeout(() => {
        navigate('/admin/seniorities');
      }, 1500);
    } catch (err) {
      setError(err.response?.data?.message || 'Error al crear seniority');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Nuevo Seniority</h1>
          <p className="text-slate-600 mt-1">
            Crea un nuevo nivel de experiencia para el equipo
          </p>
        </div>
      </div>
      ...
      <div className="card-base p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-200">
            ...
            >
              {loading ? 'Creando...' : 'Crear Seniority'}
            </button>
            <button
              type="button"
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>💡 Información:</strong> Los seniorities se utilizan para clasificar a los miembros del equipo por nivel de experiencia.
          Ejemplos comunes: Trainee (1), Junior (2), Semi-Senior (3), Senior (4), Lead (5).
        </p>
      </div>
    </div>
  );
