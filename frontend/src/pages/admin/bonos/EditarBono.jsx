...
export default function EditarBono() {
  const { id } = useParams();
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const [bonoRes, monedasRes] = await Promise.all([
        bonosService.obtener(id),
        monedasService.listar({ activo: true })
      ]);
      
      const bonoData = bonoRes.bono;
      setBono(bonoData);
      setMonedas(monedasRes.monedas || []);
      
      setFormData({
        nombre: bonoData.nombre,
        descripcion: bonoData.descripcion || '',
        monto: bonoData.monto.toString(),
        moneda_id: bonoData.moneda_id.toString(),
        periodo: bonoData.periodo,
        fecha_inicio: bonoData.fecha_inicio ? bonoData.fecha_inicio.split('T')[0] : '',
        fecha_fin: bonoData.fecha_fin ? bonoData.fecha_fin.split('T')[0] : '',
        activo: bonoData.activo,
      });
    } catch (error) {
      console.error('Error al cargar datos:', error);
      setError('Error al cargar bono');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await bonosService.actualizar(id, {
        ...formData,
        monto: parseFloat(formData.monto),
      });
      setSuccess('Bono actualizado exitosamente');
      setTimeout(() => {
        navigate('/admin/bonos');
      }, 1500);
    } catch (err) {
      setError(err.response?.data?.message || 'Error al actualizar bono');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Editar Bono</h1>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>💡 Información:</strong> Los cambios se reflejarán inmediatamente en los cortes mensuales.
          Si el bono ya está asignado a usuarios, se actualizará automáticamente.
        </p>
      </div>
    </div>
  );
